    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            @if (auth()->user()->roles_id == 1)
                <form method="POST" action="{{ route('admin.tanding.destroy-all') }}" enctype="multipart/form-data">
                @elseif (auth()->user()->roles_id == 2)
                    <form method="POST" action="{{ route('op.tanding.destroy-all') }}" enctype="multipart/form-data">
            @endif
            @csrf
            @method('delete')
