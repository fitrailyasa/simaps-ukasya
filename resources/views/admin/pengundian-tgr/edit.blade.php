...
<!-- Modal -->
<div class="modal fade bd-example-modal-sm{{ $pengundiantgr->id }}" tabindex="-1" role="dialog" aria-hidden="">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            @if (auth()->user()->roles_id == 1)
                <form method="POST" action="{{ route('admin.pengundian-tgr.update', $pengundiantgr->id) }}"
                    enctype="multipart/form-data">
                @elseif (auth()->user()->roles_id == 2)
                    <form method="POST" action="{{ route('op.pengundian-tgr.update', $pengundiantgr->id) }}"
                        enctype="multipart/form-data">
            @endif
            @csrf
            @method('PUT')
                    ...
                    <div class="col-md-6">
                        <div class="mb-2">
                            ...
                            <select class="form-select @error('atlet_id') is-invalid @enderror" name="atlet_id"
                                id="atlet_id" value="{{ $pengundiantgr->atlet_id }}" disabled>
