...
<!-- Modal -->
<div class="modal fade formEdit{{ $gelanggang->id }}" tabindex="-1" role="dialog" aria-hidden="">
        ...
        <div class="modal-content">
            ...
            <div class="modal-footer">
                @if (auth()->user()->roles_id == 1)
                    <form method="POST" action="{{ route('admin.gelanggang.destroy', $gelanggang->id) }}"
                        enctype="multipart/form-data">
                    @elseif (auth()->user()->roles_id == 2)
                        <form method="POST" action="{{ route('op.gelanggang.destroy', $gelanggang->id) }}"
                            enctype="multipart/form-data">
                @endif
                @method('DELETE')
                @csrf
