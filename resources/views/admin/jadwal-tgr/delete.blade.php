...
<!-- Modal -->
<div class="modal fade formEdit{{ $jadwaltgr->id }}" tabindex="-1" role="dialog" aria-hidden="">
        ...
        <div class="modal-content">
            ...
            <div class="modal-footer">
                @if (auth()->user()->roles_id == 1)
                    <form method="POST" action="{{ route('admin.jadwal-tgr.destroy', $jadwaltgr->id) }}"
                        enctype="multipart/form-data">
                    @elseif (auth()->user()->roles_id == 2)
                        <form method="POST" action="{{ route('op.jadwal-tgr.destroy', $jadwaltgr->id) }}"
                            enctype="multipart/form-data">
                @endif
                @method('DELETE')
                @csrf
