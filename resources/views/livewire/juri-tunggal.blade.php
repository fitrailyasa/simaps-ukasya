<div class="pt-4">
    ...
    <div id="jadwal" data-id="{{ $jadwal->id }}" gelanggang-id={{ $gelanggang->id }} class="container-fluid pb-4 "
        ...
        <div class="header d-flex justify-content-between p-1 mt-1" style="color: black">
            <div class="nama-petarung" style="width: 40%">
                <span class="fw-bold" style="font-size: 1.3rem">{{ $tampil->region }}</span>
                <h6 class="fw-bold"
                    style="{{ $tampil->id == $pengundian_biru->atlet_id ? 'color: #0053a6 !important' : 'color:  red!important' }}">
                    {{ $tampil->nama }} , {{ $tampil->kontingen }}
                </h6>
            </div>
            <h3 class="fw-bold">{{ $juri->permissions }}</h3>
            <div class="jenis-lomba text-end" style="width: 40%">
                <span class="fw-bold" style="font-size: 1.3rem">{{ $gelanggang->nama }}, Match {{ $jadwal->partai }}</span>
                <h3 class="fw-bold">Tunggal</h3>
            </div>
        </div>
        <div class="content">
            <div class="header text-center border border-secondary p-1"
                style="width: 100%; font-weight: 600; font-size: 1.3rem">
                Tunggal Jurus 1 Tangan Kosong Movement 1
            </div>
            <div class="content">
                <div class="tombol d-flex gap-1" style="width: 100%; min-height: 20vh">
                    ...
                    <div class="requirement text-center d-flex flex-column  justify-content-center fw-bold border border-secondary"
                        style="width: 20%">
                        <div class="tombol d-flex  justify-content-center">
                            <button id="fullscreen-btn" class="btn btn-primary"
                                style="width: 85px; height: 85px;display: flex;align-items: center;justify-content: center;">
            ...
            <div class="footer mt-1 d-flex flex-column gap-1" style="height: 20vh; width: 100%">
                <div class="accuracy-score d-flex gap-1">
                    <div class="main text-center fw-bold border border-dark"
                        style="width: 90%; background-color: #ececec">
                        <span>ACCURACY TOTAL SCORE</span>
                    </div>
                    <div class="aside fw-bold text-center border border-dark" style="width: 10%; font-size: 1.25rem">
                        <span
                            style="color: #0053a6">{{ $penilaian_tunggal ? number_format(9.9 - $penilaian_tunggal->salah * 0.01, 2) : '9.90' }}</span>
                    </div>
                </div>
                <div class="range-score d-flex gap-1" style="min-height: 20vh">
                    <div class="main text-center fw-bold border border-dark d-flex flex-column justify-content-center"
                        style="width: 90%;">
                        <p class="fw-bold">
                            FLOW OF MOVEMENT / STAMINA RANGE SCORE: 0.01 -
                            0.10</p>
                        <div class="score-detail d-flex gap-1 justify-content-center pb-1">
                            @for ($i = 1; $i <= 10; $i++)
                                @if ($i == 10)
                                    <button wire:click='tambahNilaiTrigger({{ $tampil->id }},{{ $i }})'f
                                ...
                                @else
                                    <button wire:click='tambahNilaiTrigger({{ $tampil->id }},{{ $i }})'
                                        class="p-1 {{ $active == $i / 100 ? 'btn-danger' : 'btn-primary' }}"
                                        style=" width:9%; height: 50px;border-radius: 8px;">
                                        <h3 class="fw-bold">
                                            0.0{{ $i }}
                    ...
                    <div class="aside fw-bold text-center border border-dark d-flex flex-column justify-content-center"
                        style="width: 10%; font-size: 1.25rem">
                        <h3 class="fw-bold" style="color: #0053a6">
                            {{ $penilaian_tunggal ? number_format($penilaian_tunggal->flow_skor, 2) : '0' }} </h3>
                    </div>
                </div>
                <div class="total-score d-flex gap-1">
                    ...
                    <div class="aside fw-bold text-center border border-dark" style="width: 10%; font-size: 1.25rem">
                        <span style="color: #0053a6">
                            {{ $penilaian_tunggal ? number_format($penilaian_tunggal->skor, 2) : '9.90' }}
                        </span>
                    </div>
                </div>
            </div>
